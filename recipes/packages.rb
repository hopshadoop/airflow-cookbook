...
bash 'patch_for_rondb_max_row_size' do
  ...
  user "root"
    code <<-EOF
      set -e
      sed -i 's/5000/1000/' #{node['conda']['base_dir']}/envs/airflow/lib/python#{node['airflow']['python']['version']}/site-packages/airflow/migrations/versions/fe461863935f_increase_length_for_connection_password.py
    EOF
end
